...
// 관리자 승인 기반 사용자 시스템 + 권한 부여 기능 + 승인/거부/삭제 버튼 + 전화번호 입력 및 사용자 정보 표시 + 관리자 직접 ID/PW 생성 + 비밀번호 표시 기능 추가 + 삭제 및 위치 변경 가능 + GOREA 파란색 로고 포함 + 중앙정렬 + 반응형
                ...
                <ul className="space-y-2">
                  {users.filter(u => !u.approved).map((u, i) => (
                    <li key={i} className="text-gray-700">
                      ...
                      <button className="ml-2 px-2 py-1 bg-green-500 text-white text-sm rounded hover:bg-green-600" onClick={() => approveUser(u.email, u.role)}>승인</button>
                      <button className="ml-2 px-2 py-1 bg-red-500 text-white text-sm rounded hover:bg-red-600" onClick={() => rejectUser(u.email)}>삭제</button>
                      <button className="ml-2 px-2 py-1 bg-gray-300 text-sm rounded" onClick={() => moveUser(i, -1)}>⬆</button>
                      <button className="ml-1 px-2 py-1 bg-gray-300 text-sm rounded" onClick={() => moveUser(i, 1)}>⬇</button>
                    </li>
                  ))}
                </ul>
              </div>
